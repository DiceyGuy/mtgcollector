...
class MTGApiService {
    ...
    async searchCards(query, filters = {}) {
        try {
            ...
            if (colors) searchQuery += ` color:${colors}`;
            if (set) searchQuery += ` set:${set}`;
            if (rarity) searchQuery += ` rarity:${rarity}`;
            if (cmc) searchQuery += ` cmc:${cmc}`;
            
            const encodedQuery = encodeURIComponent(searchQuery);
            const url = `${this.baseUrl}/cards/search?q=${encodedQuery}&page=${page}`;
            
            const response = await this.makeRequest(url);
            
            if (response && response.object === 'list') {
                return {
                    cards: response.data,
    ...
    async getRandomCard(query = '') {
        try {
            let url = `${this.baseUrl}/cards/random`;
    ...
    async getSets() {
        try {
            ...
            if (this.cache.has(cacheKey)) {
                const cached = this.cache.get(cacheKey);
                if (Date.now() - cached.timestamp < this.cacheExpiry * 10) { // Sets change less frequently
                    return cached.data;
                }
            }
    ...
    formatCardForDataset(card, matchInfo = {}) {
        return {
            ...
            // Legalities
            legalities: this.getCardLegalities(card),
